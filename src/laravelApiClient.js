...
const api = axios.create({
  baseURL: process.env.EXPO_PUBLIC_API_BASE_URL, // e.g. https://tico.getmysolutions.in/api/v1
  timeout: 60000,
...
api.interceptors.request.use(async (config) => {
  ...
  // ✅ Fix FormData boundary issues (uploads / multipart)
  const isFormData =
    typeof FormData !== "undefined" && config.data instanceof FormData;
  ...
  if (isFormData) {
    ...
    // For plain JSON, it's safe to set
