...
export default function DashboardScreen({ navigation }) {
  ...
  const computed = useMemo(() => {
    ...
    const clockIn = a.clock_in;  // "09:50:33"
    const clockOut = a.clock_out; // "00:00:00" if not clocked out

    // Worked seconds:
    // if clockOut is 00:00:00 -> (now - today clock_in)
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
      ...
      {/* Your existing welcome / quick actions */}
      <Card style={{ marginBottom: 12 }}>
        <Card.Title title={`Hi, ${user?.name ?? "User"} 👋`} subtitle="Quick actions" />
        <Card.Content>
